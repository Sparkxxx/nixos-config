  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.shinobi = {
      ...
      */
      serviceConfig = let
        configJSONFile = let
          upstreamJSON = builtins.fromJSON (builtins.readFile "${pkgs.shinobi}/lib/node_modules/shinobi/conf.sample.json");
          ...
          # So, here we are.
        in
          settingsFormat.generate "shinobi-conf.json" (lib.recursiveUpdate (builtins.removeAttrs upstreamJSON ["addStorage"]) cfg.settings // {ffmpegDir = "${cfg.ffmpegPackage}/bin/ffmpeg";});
        superUsersFile =
